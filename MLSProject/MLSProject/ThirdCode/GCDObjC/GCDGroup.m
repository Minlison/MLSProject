...
- (void)notify:(void (^)())completion
{
        [self notifyInQueue:dispatch_get_main_queue() completion:completion];
}
- (void)notifyInQueue:(dispatch_queue_t)queue completion:(void (^)())completion
{
        dispatch_group_notify(self.dispatchGroup, queue, completion);
}
@end
