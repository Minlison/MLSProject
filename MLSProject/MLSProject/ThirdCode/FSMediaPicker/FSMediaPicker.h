...
typedef enum {
        FSEditModeStandard = 0, // 正方形
        FSEditModeCircular = 1, // 圆形
        FSEditModeClear    = 2, // 没有遮罩
	FSEditModeScaleEdit = 3, // 根据比例进行编辑 默认 1:1
        FSEditModeNone     = 4, // 无法编辑，直接选择
} FSEditMode;
...
@optional
- (void)mediaPicker:(FSMediaPicker *)mediaPicker failedAuth:(NSError *)error;
...
@property (assign, nonatomic) CGSize editScale;

@property (weak, nonatomic) id<FSMediaPickerDelegate> delegate;

@property (copy, nonatomic) BOOL(^failedPresentImagePickerBlock)(FSMediaPicker *mediaPicker,NSError *error);
...
@property (readonly, nonatomic) UIImage      *scaleEditedImage;
