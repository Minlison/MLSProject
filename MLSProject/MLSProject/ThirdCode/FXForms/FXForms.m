...
@property (nonatomic, strong, readwrite) NSDictionary *fieldTemplate;
@property (nonatomic, assign, readwrite) BOOL isSortable;
@property (nonatomic, assign, readwrite) BOOL isInline;
@property (nonatomic, copy, readonly) id (^valueTransformer)(id input);
@property (nonatomic, copy, readonly) id (^reverseValueTransformer)(id input);
@property (nonatomic, strong) id defaultValue;
@property (nonatomic, strong) id header;
@property (nonatomic, strong) id footer;

@property (nonatomic, readwrite, weak) FXFormController *formController;
@property (nonatomic, readwrite, strong) NSMutableDictionary *cellConfig;
...
- (void)setOptionSelected:(BOOL)selected atIndex:(NSUInteger)index
{
        ...
        if ([self isCollectionType])
        {
                ...
                else if ([self.valueClass isSubclassOfClass:[NSArray class]])
                {
                        ...
                        [self.options enumerateObjectsUsingBlock:^(id option, NSUInteger i, __unused BOOL *stop) {
                                ...
                                if (i == index)
                                {
                                        if (selected) [(NSMutableArray *)collection addObject:option];
                                }
                                else if ([self.value containsObject:option])
                                {
                                        [(NSMutableArray *)collection addObject:option];
                                }
                        }];
                }
...
- (void)updateFormValue
{
        ...
        {
                [(NSMutableArray *)collection addObjectsFromArray:self.values];
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)index
{
        ...
        if ([header isKindOfClass:[UIView class]])
        {
                return header.frame.size.height ?: CGFLOAT_MIN;
        }
        return CGFLOAT_MIN;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)index
{
        ...
        if ([footer isKindOfClass:[UIView class]])
        {
                return footer.frame.size.height ?: CGFLOAT_MIN;
        }
        return CGFLOAT_MIN;
}
...
@interface FXFormTextFieldCell () <US2ValidatorUIDelegate, QMUITextFieldDelegate>

@property (nonatomic, readwrite, strong) US2ValidatorTextField *textField;
...
- (void)setUp
{
        ...
        self.textField = [[US2ValidatorTextField alloc] initWithFrame:CGRectMake(0, 0, 200, 21)];
        ...
        self.textField.validatorUIDelegate = self;
...
- (void)textFieldDidEndEditing:(__unused US2ValidatorTextField *)textField
{
        [self updateFieldValue:textField.isValid];
        if ([self autoAction])
        {
                if (self.field.action) self.field.action(self);
        }
...
- (void)updateFieldValue:(BOOL)isValid
{
        self.field.value = isValid ? self.textField.text : nil;
}
