...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < self.searchResult.count) {
        ...
        if (self.searchResult.count == 1) text = [NSString stringWithFormat:NSLocalizedStringWithDefaultValue(@"Search for \"%@\"",@"DZNPhotoPickerController",[NSBundle mainBundle],nil,@""), tag.term];
        else text = tag.term;
    }
