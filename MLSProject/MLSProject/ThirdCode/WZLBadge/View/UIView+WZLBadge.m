...
{
        [self showBadgeWithStyle:style text:@(value).stringValue animationType:aniType];
}
             ...
             animationType:(WBadgeAnimType)aniType
{
        ...
        switch (style) {
                ...
                case WBadgeStyleNumber:
                        [self showNumberBadgeWithString:text];
                        break;
                case WBadgeStyleNew:
                        [self showNewBadge];
                        break;
...
- (void)showNumberBadgeWithText:(NSString *)text
                  animationType:(WBadgeAnimType)aniType
{
        self.aniType = aniType;
        [self showNumberBadgeWithString:text];
...
- (void)resetBadgeForRedDot {
    if (self.badgeRadius > 0) {
        self.badge.bounds = CGRectMake(0, 0 , MAX(self.badgeFrame.size.width, self.badgeRadius *2), MAX(self.badgeFrame.size.height, self.badgeRadius *2));
            self.badge.center = self.badge.center;
    }
}
...
- (void)showNumberBadgeWithString:(NSString *)value
{
        if (value < 0) {
                return;
        }
        ...
        if (value.integerValue > self.badgeMaximumBadgeNumber)
        {
                
        }
        
        self.badge.text = (value.integerValue > self.badgeMaximumBadgeNumber ?
        ...
        CGRect badgeBounds = CGRectMake(0, 0, MAX(frame.size.width, self.badgeRadius * 2), MAX(frame.size.height, self.badgeRadius * 2));
//        self.badge.center = CGPointMake(CGRectGetWidth(self.frame) + 2 + self.badgeCenterOffset.x, self.badgeCenterOffset.y);
        self.badge.center = CGPointMake(CGRectGetWidth(self.frame) + self.badgeCenterOffset.x,  (self.badgeCenterOffset.y));
        self.badge.bounds = badgeBounds;
        self.badge.layer.cornerRadius = CGRectGetHeight(self.badge.frame) * 0.5;
}
- (void)showNumberBadgeWithValue:(NSInteger)value
{
        [self showNumberBadgeWithString:@(value).stringValue];
}
...
//            CGFloat radius = [self badgeRadius];
            self.badge.layer.cornerRadius = CGRectGetHeight(self.badge.frame) * 0.5;
...
#pragma clang diagnostic pop
	...
	frame.size = CGSizeMake(MAX(ceilf(labelsize.width), [self badgeFrame].size.width), MAX(ceilf(labelsize.height), [self badgeFrame].size.height));
