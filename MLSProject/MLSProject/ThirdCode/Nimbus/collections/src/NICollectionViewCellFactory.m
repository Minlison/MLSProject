...
- (NSString *)identifierForObj:(id)object collectionView:(UICollectionView *)collectionView
{
        NSString *identifier = [self.cellClassIdentifierMap objectForKey:NSStringFromClass([object collectionViewCellClass])];
        if (identifier)
        {
                return identifier;
        }
        identifier = NSStringFromClass([object collectionViewCellClass]);
        // 快速创建的对象,拼接class
        if ([object conformsToProtocol:@protocol(NICollectionViewNibCellObject)])
        {
                id <NICollectionViewNibCellObject>cellObj = object;
                identifier = [identifier stringByAppendingString:NSStringFromClass([cellObj collectionViewCellClass])];
                [collectionView registerNib:[cellObj collectionViewCellNib] forCellWithReuseIdentifier:identifier];
        }
        else if ([object conformsToProtocol:@protocol(NICollectionViewCellObject)]){
                id <NICollectionViewCellObject>cellObject = object;
                Class cellClass = [cellObject collectionViewCellClass];
        ...
        [self.cellClassIdentifierMap setObject:identifier forKey:NSStringFromClass([object collectionViewCellClass])];
        return identifier;
}
...
- (UICollectionViewCell *)cellWithClass:(Class)collectionViewCellClass
  ...
  NSString* identifier = [self identifierForObj:object collectionView:collectionView];
  ...
  if ([cell respondsToSelector:@selector(shouldUpdateCellWithObject:)]) {
    [(id<NICollectionViewCell>)cell shouldUpdateCellWithObject:object];
          ...
          [(id<NICollectionViewCell>)cell shouldUpdateCellWithObject:object atIndexPath:indexPath];
  }
...
- (CGSize)collectionView:(UICollectionView *)collectionView sizeForItemAtIndexPath:(NSIndexPath *)indexPath model:(NICollectionViewModel *)model
{
        ...
        if ([collectionView.collectionViewLayout isKindOfClass:[UICollectionViewFlowLayout class]]) {
                itemSize = [(UICollectionViewFlowLayout *)(collectionView.collectionViewLayout) itemSize];
        }
        id object = [model objectAtIndexPath:indexPath];
        Class cellClass = [self collectionViewCellClassFromObject:object];
        
        if ([cellClass respondsToSelector:@selector(sizeForObject:identifier:atIndexPath:collectionView:)]) {
                CGSize cellSize = [cellClass sizeForObject:object
                                                     identifier:[self identifierForObj:object collectionView:collectionView]
                                                    atIndexPath:indexPath collectionView:collectionView];
...
@implementation NINibCollectionViewCellObject

// Designated initializer.
- (id)initWithCellNib:(UINib *)cellNib cellClass:(Class)cellClass userInfo:(id)userInfo
{
        if (self = [super init]) {
                _collectionViewCellNib = cellNib;
                _collectionViewCellClass = cellClass;
                _userInfo = userInfo;
        }
        return self;
}
