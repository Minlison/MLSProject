...
- (void)qmui_layoutSublayersOfLayer:(CALayer *)layer {
    ...
    if (!self.qmui_borderLayer) {
        ...
        [self.layer insertSublayer:self.qmui_borderLayer atIndex:0];
    }
    ...
    if (self.qmui_borderPosition & QMUIBorderViewPositionRight) {
        ...
        {
                UIColor *fillColor = self.backgroundColor ? self.backgroundColor : [UIColor whiteColor];
                self.qmui_borderLayer.shadowOffset = CGSizeMake(0, 0);
                self.qmui_borderLayer.shadowColor = self.qmui_shadowColor.CGColor;
                self.qmui_borderLayer.shadowRadius = self.qmui_borderWidth;
                self.qmui_borderLayer.shadowOpacity = self.qmui_shadowOpacity;
                self.qmui_borderLayer.fillColor = fillColor.CGColor;
                self.qmui_borderLayer.backgroundColor = fillColor.CGColor;
                self.qmui_borderLayer.shadowPath = [UIBezierPath bezierPathWithRect:CGRectInsetEdges(self.bounds, UIEdgeInsetsMake(-borderWidth, -borderWidth, -borderWidth, -borderWidth))].CGPath;
...
static char kAssociatedObjectKey_enableShadow;
- (void)setQmui_enableShadow:(BOOL)qmui_enableShadow
{
        objc_setAssociatedObject(self, &kAssociatedObjectKey_enableShadow, @(qmui_enableShadow), OBJC_ASSOCIATION_ASSIGN);
        [self setNeedsLayout];
}
- (BOOL)qmui_enableShadow
{
        return (BOOL)[objc_getAssociatedObject(self, &kAssociatedObjectKey_enableShadow) boolValue];
}
static char kAssociatedObjectKey_shadowOpacity;
- (void)setQmui_shadowOpacity:(CGFloat)qmui_shadowOpacity
{
        objc_setAssociatedObject(self, &kAssociatedObjectKey_shadowOpacity, @(qmui_shadowOpacity), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        [self setNeedsLayout];
}
- (CGFloat)qmui_shadowOpacity
{
        return (CGFloat)[objc_getAssociatedObject(self, &kAssociatedObjectKey_shadowOpacity) floatValue];
}
static char kAssociatedObjectKey_shadowColor;
- (void)setQmui_shadowColor:(UIColor *)qmui_shadowColor {
        objc_setAssociatedObject(self, &kAssociatedObjectKey_shadowColor, qmui_shadowColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        [self setNeedsLayout];
}

- (UIColor *)qmui_shadowColor {
        return (UIColor *)objc_getAssociatedObject(self, &kAssociatedObjectKey_shadowColor);
}
