...
@property(nonatomic, strong) WZProtocolInterceptor <QMUITextViewDelegate> *delegateProxy;
...
- (void)handleTextChanged:(id)sender {
    ...
    if (textView) {
        ...
        if (self.autoResizable) {
            ...
            if ([textView.delegateProxy.receiver respondsToSelector:@selector(textView:newHeightAfterTextChanged:)] && resultHeight != CGRectGetHeight(self.bounds)) {
                [textView.delegateProxy.receiver textView:self newHeightAfterTextChanged:resultHeight];
            }
        }
...
- (BOOL)textView:(QMUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if (textView.maximumTextLength < NSUIntegerMax) {
        ...
        if (isDeleting || textView.markedTextRange) {
            ...
            if ([textView.delegateProxy.receiver respondsToSelector:_cmd]) {
                return [textView.delegateProxy.receiver textView:textView shouldChangeTextInRange:range replacementText:text];
            }
        ...
        if (textWillOutofMaximumTextLength) {
            ...
            if ([self lengthWithString:textView.text] - rangeLength == textView.maximumTextLength && [text isEqualToString:@"\n"]) {
                if ([textView.delegateProxy.receiver respondsToSelector:_cmd]) {
                    ...
                    [textView.delegateProxy.receiver textView:textView shouldChangeTextInRange:range replacementText:text];
                    return NO;
                }
            }
            ...
            if (substringLength > 0 && [self lengthWithString:text] > substringLength) {
                ...
                if ([self lengthWithString:allowedText] <= substringLength) {
                    ...
                    if (!textView.shouldResponseToProgrammaticallyTextChanges) {
                        [textView.delegateProxy.receiver textViewDidChange:textView];
                    }
                }
            }
            
            if ([self.delegateProxy.receiver respondsToSelector:@selector(textView:didPreventTextChangeInRange:replacementText:)]) {
                [self.delegateProxy.receiver textView:textView didPreventTextChangeInRange:range replacementText:text];
            }
            return NO;
        }
    }
    ...
    if (!textView.markedTextRange) {
        if ([self lengthWithString:textView.text] > textView.maximumTextLength) {
            ...
            if ([self.delegateProxy.receiver respondsToSelector:@selector(textView:didPreventTextChangeInRange:replacementText:)]) {
                ...
                [self.delegateProxy.receiver textView:textView didPreventTextChangeInRange:textView.selectedRange replacementText:nil];
            }
...
- (void)setDelegate:(id<QMUITextViewDelegate>)delegate {
        ...
        self.delegateProxy = (WZProtocolInterceptor<QMUITextViewDelegate>*)[[WZProtocolInterceptor alloc] initWithInterceptedProtocol:@protocol(QMUITextViewDelegate)];
        self.delegateProxy.receiver = delegate;
        self.delegateProxy.middleMan = self;
        [super setDelegate:self.delegateProxy];
}

// 下面这两个方法比较特殊，无法通过 forwardInvocation: 的方式把消息发送给 self.delegateProxy.receiver，只会直接被调用，所以只能在 QMUITextView 内部实现这连个方法然后调用 delegateProxy.receiver 的对应方法
...
- (void)scrollViewDidZoom:(UIScrollView *)scrollView {
    if ([self.delegateProxy.receiver respondsToSelector:_cmd]) {
        [self.delegateProxy.receiver scrollViewDidZoom:scrollView];
    }
}
