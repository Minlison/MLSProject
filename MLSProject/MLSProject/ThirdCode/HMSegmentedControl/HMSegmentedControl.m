	...
	if (self.type == HMSegmentedControlTypeText) {
		[self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
			...
			if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
				rect = CGRectMake((self.segmentWidth * idx) + (self.segmentWidth - stringWidth) / 2, y, stringWidth, stringHeight);
                                rectDiv = CGRectMake((self.segmentWidth * idx) - (self.verticalDividerWidth / 2), self.verticalDividerHeight == 0 ? self.selectionIndicatorHeight * 2 :(self.frame.size.height - self.verticalDividerHeight)/2, self.verticalDividerWidth,self.verticalDividerHeight == 0 ? self.frame.size.height - (self.selectionIndicatorHeight * 4) : self.verticalDividerHeight);
				...
				rectDiv = CGRectMake(xOffset - (self.verticalDividerWidth / 2), self.verticalDividerHeight == 0 ? self.selectionIndicatorHeight * 2 :(self.frame.size.height - self.verticalDividerHeight)/2, self.verticalDividerWidth, self.verticalDividerHeight == 0 ? self.frame.size.height - (self.selectionIndicatorHeight * 4) : self.verticalDividerHeight);
			}
			...
			if (self.showDotBlock && self.showDotBlock(idx)) {
				
				CATextLayer *dotLayer = [CATextLayer layer];
				dotLayer.backgroundColor = self.dotColor.CGColor;
				dotLayer.bounds = CGRectMake(0, 0, self.dotSize.width, self.dotSize.height);
				dotLayer.position = CGPointMake(CGRectGetWidth(titleLayer.bounds), 0);
				dotLayer.cornerRadius = self.dotSize.height * 0.5;
				dotLayer.masksToBounds = YES;
				
				[titleLayer addSublayer:dotLayer];
			}
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
			...
			if (self.showDotBlock && self.showDotBlock(idx)) {
				...
				dotLayer.backgroundColor = self.dotColor.CGColor;
				dotLayer.bounds = CGRectMake(0, 0, self.dotSize.width, self.dotSize.height);
				dotLayer.position = CGPointMake(CGRectGetWidth(imageLayer.bounds), 0);
				dotLayer.cornerRadius = self.dotSize.height * 0.5;
				dotLayer.masksToBounds = YES;
				
				[imageLayer addSublayer:dotLayer];
			}
	...
	if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
		...
		if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
			...
			if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
				...
				NSInteger i = 0;
				CGFloat selectedWith = 0.0;
				for (NSNumber *width in self.segmentWidthsArray) {
					if (self.selectedSegmentIndex == i)
					{
						selectedWith = width.floatValue;
						break;
					}
					selectedSegmentOffset = selectedSegmentOffset + [width floatValue];
					i++;
				}
				CGFloat textStripeOffset = 0.0;
				if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe)
				{
					CGSize size = [self measureTitleAtIndex:self.selectedSegmentIndex];
					textStripeOffset = (selectedWith - size.width) * 0.5;
				...
				@try {
					...
					/// MARK: - 修改 MinLison
					/// 防止底部 indicator 缩短
					return CGRectMake(selectedSegmentOffset + self.selectionIndicatorEdgeInsets.left + textStripeOffset, indicatorYOffset, [[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue] - self.selectionIndicatorEdgeInsets.right - self.selectionIndicatorEdgeInsets.left - textStripeOffset * 2, self.selectionIndicatorHeight + self.selectionIndicatorEdgeInsets.bottom);
			...
			return CGRectMake((self.segmentWidth + self.selectionIndicatorEdgeInsets.left) * self.selectedSegmentIndex, indicatorYOffset, self.segmentWidth - self.selectionIndicatorEdgeInsets.right - self.selectionIndicatorEdgeInsets.left, self.selectionIndicatorHeight);
		}
	}
}
...
- (void)updateSegmentsRects {
	...
	if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
		...
		NSMutableArray *mutableSegmentWidths = [NSMutableArray arrayWithCapacity:1];
