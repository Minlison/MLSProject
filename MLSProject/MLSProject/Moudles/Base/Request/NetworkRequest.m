...
- (void)configParams
{
        if ([self.subClassHoldertDelegate respondsToSelector:@selector(requestParams)])
        {
                NSDictionary *dict = [self.subClassHoldertDelegate requestParams];
                if (dict)
                {
        ...
        {
                self.encryptParams = self.params;
        }
}
...
{
        if (self.subClassHoldertDelegate && [self.subClassHoldertDelegate respondsToSelector:@selector(requestTimeoutInterval)]) {
                return [self.subClassHoldertDelegate requestTimeoutInterval];
        }
        return 10.0;
}
- (NSString *)baseUrl
...
{
        NSArray *array = nil;
        if (self.subClassHoldertDelegate && [self.subClassHoldertDelegate respondsToSelector:@selector(requestAuthorizationHeaderFieldArray)]) {
                array = [self.subClassHoldertDelegate requestAuthorizationHeaderFieldArray];
        }
        return array;
}
- (nullable NSSet <NSString *>*)acceptableContentTypes
{
        NSSet *acceptableContentTypes = nil;
        if (self.subClassHoldertDelegate && [self.subClassHoldertDelegate respondsToSelector:@selector(acceptableContentTypes)]) {
                acceptableContentTypes = [self.subClassHoldertDelegate acceptableContentTypes];
        }
...
- (nullable NSString *)queryStringSerializationRequest:(NSURLRequest *)request param:(NSDictionary *)param error:(NSError **)error
{
        ...
        if (self.subClassHoldertDelegate && [self.subClassHoldertDelegate respondsToSelector:@selector(queryStringSerializationRequest:param:error:)]) {
                queryString = [self.subClassHoldertDelegate queryStringSerializationRequest:request param:param error:error];
        }
        return queryString;
}
- (nullable NSDictionary<NSString *, NSString *> *)requestHeaderFieldValueDictionary
{
        ...
        if (self.subClassHoldertDelegate && [self.subClassHoldertDelegate respondsToSelector:@selector(requestHeaderFieldValueDictionary)]) {
                 NSDictionary *delDict = [self.subClassHoldertDelegate requestHeaderFieldValueDictionary];
                if (delDict) {
                        [dict addEntriesFromDictionary:delDict];
                }
        }
        return dict;
}
...
- (NSString *)cacheIdentifier
{
        ...
        [dict removeObjectForKey:kRequestKeyTrace_Id];
        [dict removeObjectForKey:kRequestKeyNetType];
        return [[[dict jk_URLQueryStringWithSortedKeys:YES] stringByAppendingString:self.baseUrl] stringByAppendingString:NOT_NULL_STRING_DEFAULT_EMPTY(self.url)].md5String;
}
- (NSString *)description
{
        ...
        if ( self.error )
        {
                return [NSString stringWithFormat:@"{<%@: %p>\n== 失败 ==\n接口===%@\n请求头===%@\n参数===%@\n加密后参数===%@\n错误信息===%@\n内容===%@\n重试次数===%d\n}", NSStringFromClass([self class]), self,self.originalRequest.URL.absoluteString,self.originalRequest.allHTTPHeaderFields,self.params,self.encryptParams,self.error,[[self.responseString jsonValueDecoded] jsonPrettyStringEncoded],(int)self.failedRetryCount];
        }
        /// 成功
        if ( [self isDataFromCache] )
        {
                return [NSString stringWithFormat:@"{<%@: %p>\n== 缓存内容 ==\n接口===%@\n请求头===%@\n参数===%@\n内容===%@\n重试次数===%d\n}", NSStringFromClass([self class]), self,self.originalRequest.URL.absoluteString,self.originalRequest.allHTTPHeaderFields,self.params,self.decryptResponseString,(int)self.failedRetryCount];
        }
        return [NSString stringWithFormat:@"{<%@: %p>\n== 网络内容 ==\n接口===%@\n请求头===%@\n参数===%@\n加密后参数===%@\n内容===%@\n重试次数===%d\n}", NSStringFromClass([self class]), self,self.originalRequest.URL.absoluteString,self.originalRequest.allHTTPHeaderFields,self.params,self.encryptParams,[[self.decryptResponseString jsonValueDecoded] jsonPrettyStringEncoded],(int)self.failedRetryCount];
}
