...
#pragma mark - Global Color
        ...
        QMUICMI.clearColor = [UIColor clearColor];       // UIColorClear : 透明色
        QMUICMI.whiteColor = [UIColor whiteColor];       // UIColorWhite : 白色（不用 [UIColor whiteColor] 是希望保持颜色空间为 RGB）
        QMUICMI.blackColor = UIColorHex(0x333333); // [UIColor colorWithRed:0 green:0 blue:0 alpha:1];       // UIColorBlack : 黑色（不用 [UIColor blackColor] 是希望保持颜色空间为 RGB）
        QMUICMI.grayColor = UIColorHex(0x9e9e9e);                                   // UIColorGray  : 最常用的灰色
        QMUICMI.grayDarkenColor = UIColorHex(0x787878);                             // UIColorGrayDarken : 深一点的灰色
        QMUICMI.grayLightenColor = UIColorHex(0xdddddd);                            // UIColorGrayLighten : 浅一点的灰色
        QMUICMI.redColor = UIColorHex(0xED432F);//UIColorMake(250, 58, 58);                                // UIColorRed : 红色
        ...
        QMUICMI.maskDarkColor = UIColorMakeWithRGBA(0, 0, 0, .3f);                 // UIColorMask : 深色的背景遮罩，默认用于 QMAlertController、QMUIDialogViewController 等弹出控件的遮罩
        QMUICMI.maskLightColor = UIColorMakeWithRGBA(255, 255, 255, .5f);           // UIColorMaskWhite : 浅色的背景遮罩，QMUIKit 里默认没用到，只是占个位
        QMUICMI.separatorColor = UIColorHex(0xe6e6e6);                        // UIColorSeparator : 全局默认的分割线颜色，默认用于列表分隔线颜色、UIView (QMUI_Border) 分隔线颜色
...
//        QMUICMI.textFieldTintColor = [UIColor blueColor];                           // TextFieldTintColor : QMUITextField、QMUITextView 的 tintColor，不影响 UIKit 的输入框
...
#pragma mark - NavigationBar
        ...
        QMUICMI.navBarBackgroundImage = [UIImage imageWithColor:UIColorWhite];      // NavBarBackgroundImage : UINavigationBar 的背景图
        ...
        QMUICMI.navBarTintColor = UIColorHex(0x070808);                                     // NavBarTintColor : UINavigationBar 的 tintColor，也即导航栏上面的按钮颜色
        QMUICMI.navBarTitleColor = UIColorHex(0x070808);                                 // NavBarTitleColor : UINavigationBar 的标题颜色，以及 QMUINavigationTitleView 的默认文字颜色
        QMUICMI.navBarTitleFont = WGBoldSystem18Font;                               // NavBarTitleFont : UINavigationBar 的标题字体，以及 QMUINavigationTitleView 的默认字体
        QMUICMI.navBarBackButtonTitlePositionAdjustment = UIOffsetZero;             // NavBarBarBackButtonTitlePositionAdjustment : 导航栏返回按钮的文字偏移
//        QMUICMI.navBarBackIndicatorImage = [UIImage nav_ic_back_blackRenderingMode:(UIImageRenderingModeAlwaysOriginal)];                                     // NavBarBackIndicatorImage : 导航栏的返回按钮的图片 设置完成，会影响系统相册的返回按钮
...
#pragma mark - TabBar
        QMUICMI.tabBarBackgroundImage = [UIImage qmui_imageWithColor:[UIColor whiteColor]];   // TabBarBackgroundImage : UITabBar 的背景图
        QMUICMI.tabBarBarTintColor = [UIColor whiteColor];                                           // TabBarBarTintColor : UITabBar 的 barTintColor
        QMUICMI.tabBarShadowImageColor = UIColorSeparator;                          // TabBarShadowImageColor : UITabBar 的 shadowImage 的颜色，会自动创建一张 1px 高的图片
        QMUICMI.tabBarTintColor = UIColorHex(0x0190D4);                         // TabBarTintColor : UITabBar 的 tintColor
        QMUICMI.tabBarItemTitleColor = UIColorHex(0x969696);                                // TabBarItemTitleColor : 未选中的 UITabBarItem 的标题颜色
        QMUICMI.tabBarItemTitleColorSelected = UIColorHex(0x0190D4);                     // TabBarItemTitleColorSelected : 选中的 UITabBarItem 的标题颜色
        QMUICMI.tabBarItemTitleFont = WGSystem10Font;                                          // TabBarItemTitleFont : UITabBarItem 的标题字体
